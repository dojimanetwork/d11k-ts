...
import { IDL, baseToLamports, lamportsToBase } from './utils'
...
class SolanaClient implements SolanaChainClient {
  ...
  async createTransaction(walletIndex = 0, recipient: string, amount: number): Promise<web3.Transaction> {
    // Get account address
  ...
  async signAndSend(walletIndex = 0, rawTx: web3.Transaction): Promise<string> {
    ...
    const signature = await web3.sendAndConfirmTransaction(this.connection, rawTx, [account[walletIndex]])
  ...
  async getTransactionsHistory(params?: SolTxsHistoryParams): Promise<SolTxs> {
    ...
    const pubKey = new web3.PublicKey(params?.address || (await this.getAddress()))
    ...
    if (!signatures)
      throw new Error(`Failed to get transactions list (address: ${params?.address || (await this.getAddress())})`)
  ...
  async getInboundObject(): Promise<InboundAddressResult> {
    ...
    const inboundObj = data.find((res) => res.chain === 'SOL') as InboundAddressResult
    return inboundObj
  }
  ...
  async addLiquidityPool(amount: number, inboundAddress: string, dojAddress?: string) {
    const toAmount = baseToLamports(amount, SOL_DECIMAL)
  ...
  async swap(amount: number, token: SwapAssetList, inboundAddress: string, recipient: string) {
    const toAmount = baseToLamports(amount, SOL_DECIMAL)

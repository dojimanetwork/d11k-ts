...
import { ETH_DECIMAL, defaultEthInfuraRpcUrl, defaultInfuraApiKey } from './const'
import {
  EthTransferParams,
  EthTxData,
  EthTxDetailsResult,
...
class EthereumClient {
  ...
  calculateDojFee(baseGasFee: number, multiplier: number): number {
    return new BigNumber(baseGasFee).times(new BigNumber(multiplier)).toNumber()
  }
    ...
    const transaction = await this.web3.eth.accounts.signTransaction(
      {
        ...
        gas: params.fee
          ...
          : await this.estimateGasFee(params.amount, params.memo ? params.memo : undefined),
        data: params.memo ? this.web3.utils.toHex(params.memo) : undefined,
      },
      this.account.privateKey,
    )

    const transactionResult = await this.web3.eth.sendSignedTransaction(transaction.rawTransaction as string)
  ...
  async getTransactionsHistory(params: EthTxHistoryParams) {
    if (this.network === Network.DojTestnet) return null
      ...
      const convertTimestampToDate = (timestamp: number) => {
        return moment(timestamp).toDate().toUTCString()
      }
      ...
      try {
        const response: TransactionHistoryResult = await (await axios.get(requestUrl)).data
        if (response.status === '1') {
          const result: EthTxDetailsResult[] = response.result
          if (result !== undefined) {
            const resultTxs: EthTxs = {
              total: result.length,
              txs: result.map((res) => ({
                block: Number(res.blockNumber),
                date: moment(convertISOtoUTC(convertTimestampToDate(Number(res.timeStamp) * 1000))).format(
                  'DD/MM/YYYY',
                ),
                time: moment(convertISOtoUTC(convertTimestampToDate(Number(res.timeStamp) * 1000))).format('HH:mm:ss'),
                ...
                from: res.from,
                to: res.to,
                transaction_type: res.from === params.address.toLowerCase() ? 'Send | ETH' : 'Receive | ETH',
              })),
            }
            return resultTxs
          } else {
  ...
  async getInboundObject(): Promise<InboundAddressResult> {
    ...
    const inboundObj: InboundAddressResult = data.find((res) => res.chain === 'ETH') as InboundAddressResult
    return inboundObj
  }
...
//     const etherscan = this.getEtherscanProvider()
//     const response: GasOracleResponse = await etherscanAPI.getGasOracle(etherscan.baseUrl, etherscan.apiKey as string)

...
export default class ArweaveTxClient {
  ...
  async getTxsHistory(arweave: Arweave, owner: string, limit?: number) {
      ...
      if (txs.edges.length > 0) {
        ...
        txs.edges.map((res) => {
          const outerResult: OuterDataResult = {
            ...
            date:
              res.node.block.timestamp && res.node.block.timestamp !== (null || undefined)
    ...
    if (this.recipientHasNextPage === true || this.recipientHasNextPage === undefined) {
    ...
    if (outerTxsData.length !== 0) {
      const outerLodash: OuterDataResult[] = _.orderBy(outerTxsData, 'timestamp', 'desc')
      txsDataFinal.outer = outerLodash
    }
    if (innerTxsData.length !== 0) {
      const innerLodash: InnerDataResult[] = _.orderBy(innerTxsData, 'timestamp', 'desc')
      txsDataFinal.inner = innerLodash
    }
    return txsDataFinal
  }

  async getOwnersTxsQueryResult(arweave: Arweave, variables: ReqVariables): Promise<GQLTransactionsResultInterface> {
